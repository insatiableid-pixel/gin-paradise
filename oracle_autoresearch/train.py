...
TRACE_CONFIDENCE_DECAY = 0.1    # how much each trace event reduces risk multiplier
...
def compute_trace_confidence(spot: SpotRecord) -> float:
    ...
    """
    n_pickups = len(spot.known_opponent_pickups)
    n_discards = len(spot.known_opponent_discards)
    n_declines = len(spot.upcard_declines)
    n_events = n_pickups + n_discards + n_declines

    # ── Directive 117: Pickup-Specific Pressure ──
    # Heuristic: pickups represent active meld-seeking. 
    # Multiple pickups or same-suit pickups indicate high pressure.
    pickup_pressure = 0.0
    if n_pickups > 0:
        pickup_pressure = n_pickups * 0.2
        # Check for same-rank or same-suit clusters (higher threat)
        suits = [suit(c) for c in spot.known_opponent_pickups]
        ranks = [rank(c) for c in spot.known_opponent_pickups]
        for s in set(suits):
...
def compute_eval_weight(
    ...
    Args:
        ...
        my_score: hero's current match score
        opp_score: opponent's current match score
        score_diff: hero_score - opponent_score
    ...
    dynamic_weight = (DIRECT_EVAL_WEIGHT - 0.15) + (0.15 * gap_confidence) + (0.10 * trace_confidence) - (0.20 * pickup_pressure)
...
def compute_trace_confidence_multiplier(
    ...
    dynamic_floor = TRACE_CONFIDENCE_FLOOR + (0.015 * max(0, hero_deadwood - 4)) - (0.05 * progress) + (0.25 * pickup_pressure)
...
def predict_knock_probability_direct(
    ...
    if strategy is not None and USE_CFR_PRIOR:
        ...
        n_discards = len(spot.known_opponent_discards)
        n_declines = len(spot.upcard_declines)
        n_trace = n_pickups + n_discards + n_declines

        # Re-compute pickup pressure for blending calls
        pickup_pressure = 0.0
        if n_pickups > 0:
            pickup_pressure = n_pickups * 0.2
            suits = [suit(c) for c in spot.known_opponent_pickups]
