...
- The latest promoted incumbent is `gemini_gcli_0024` at score `0.6243`, with
  `DIRECT_EVAL_WEIGHT = 0.90` as the current live frontier.
- Immediate goal: exploit the local neighborhood around that frontier without
  wasting experiments on already-weakened standalone directions such as
  `DIRECT_EVAL_WEIGHT = 0.95`, `TRACE_CONFIDENCE_DECAY = 0.06`,
  `EARLY_EXIT_CONFIDENCE_THRESHOLD = 2.0`, `DW_RISK_SCALE = 0.008`, or
  `CFR_BUDGET_FRACTION = 0.05`.
...
## Phase 81 Frontier Findings (Highest-Priority Prior)

From the latest 12-experiment Gemini CLI campaign:

| Candidate | Change | Score | Delta vs incumbent | Decision | Takeaway |
|-----------|--------|-------|--------------------|----------|----------|
| gemini_gcli_0019 | `DIRECT_EVAL_WEIGHT: 0.70 -> 0.85` | 0.6189 | +0.0316 | Promoted | Direct eval gained major ground |
| gemini_gcli_0024 | `DIRECT_EVAL_WEIGHT: 0.85 -> 0.90` | 0.6243 | +0.0054 | Promoted | Further gain, but much smaller |
| gemini_gcli_0022 | `DIRECT_EVAL_WEIGHT: 0.85 -> 0.95` | 0.5762 | -0.0427 | Discarded | 0.95 overshoots badly |
| gemini_gcli_0020 | `TRACE_CONFIDENCE_DECAY: 0.08 -> 0.06` | 0.6134 | -0.0055 | Discarded | Near-miss, but still below margin |
| gemini_gcli_0023 | `TRACE_CONFIDENCE_DECAY: 0.08 -> 0.06` | 0.6132 | -0.0057 | Discarded | Repeat confirmed it is not enough alone |
| gemini_gcli_0021 | `CFR_BUDGET_FRACTION: 0.10 -> 0.05` | 0.5596 | -0.0593 | Discarded | Too little CFR budget is harmful |
| gemini_gcli_0017 | `EARLY_EXIT_CONFIDENCE_THRESHOLD: 2.5 -> 2.0` | 0.4966 | -0.0907 | Discarded | Large regression |
| gemini_gcli_0018 | `DW_RISK_SCALE: 0.012 -> 0.008` | 0.4983 | -0.0890 | Discarded | Large regression |

**Immediate implications:**
- The live frontier is `DIRECT_EVAL_WEIGHT ~= 0.90`, not `0.70`
- Trust in direct evaluation is helping, but the curve is not monotone; `0.95` was clearly too high
- Do not spend single-parameter shots re-testing `DIRECT_EVAL_WEIGHT` at `0.80`, `0.85`, or `0.95`
- Do not spend single-parameter shots lowering `TRACE_CONFIDENCE_DECAY` again unless it is part of a materially different coupled hypothesis
- Avoid repeating `EARLY_EXIT_CONFIDENCE_THRESHOLD = 2.0`, `DW_RISK_SCALE = 0.008`, or `CFR_BUDGET_FRACTION = 0.05` as standalone candidates

## Immediate Search Priorities For The Next 12-Run Block

Prefer candidates that explore the **local neighborhood around the new frontier** instead of rediscovering already-weakened directions:

1. **Local blend frontier work**
   - Small local moves around `DIRECT_EVAL_WEIGHT = 0.90`
   - If you change it again, prefer tight steps or coupled hypotheses, not big jumps

2. **Calibration that supports a high direct-eval weight**
   - `CALIBRATION_MAX_WORLDS`
   - `CALIBRATION_FRACTION`
   - Small supporting changes that improve reliability of a high-trust direct evaluator

3. **Budget allocation that complements, not fights, the 0.90 blend**
   - `EARLY_EXIT_MIN_WORLDS`
   - `N_WORLDS_BASE`
   - Possibly `CFR_BUDGET_FRACTION`, but only as a small, tightly justified coupled move near the current value

4. **Small coupled hypotheses**
   - 1-2 parameter edits are preferred
   - Good example: a local calibration change paired with the current high direct-eval regime
   - Bad example: repeating a strongly negative standalone move

## Phase 77 Ablation Findings (Still Useful Prior)
...
| DIRECT_EVAL_WEIGHT | 0.90 | blend | Direct eval vs CFR blend |
...
8. Treat recent negative single-parameter repeats as expensive; prefer new local information over re-running a known loser
...
{
  "request_id": "gcli_0035",
  "incumbent_score": 0.6293,
  "incumbent_variant": "gemini_gcli_0025",
  "incumbent_params": {
