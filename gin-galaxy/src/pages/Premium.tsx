  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className={`relative overflow-hidden rounded-2xl border p-6 ${
        isPremium
          ? "border-amber-500/40 bg-gradient-to-br from-amber-950/30 via-zinc-900 to-amber-950/20"
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className={`rounded-2xl border p-6 space-y-5 ${
          !isPremium ? "border-indigo-500/40 bg-zinc-900" : "border-zinc-800 bg-zinc-900/60"
        }`}>
          {!isPremium && (
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/30 text-xs font-medium text-indigo-300">
              Current Plan
            </div>
          )}
          <div>
            <h3 className="text-lg font-bold">Free</h3>
            <p className="text-3xl font-bold mt-1">$0 <span className="text-sm font-normal text-zinc-500">forever</span></p>
            <p className="text-sm text-zinc-400 mt-2">Coin-wagered PvP, daily check-in, practice matches</p>
          </div>
        ...
        {/* Premium Plan */}
        <div className={`relative rounded-2xl border p-6 space-y-5 ${
          isPremium
            ? "border-amber-500/40 bg-gradient-to-br from-amber-950/20 via-zinc-900 to-zinc-900"
          ...
          <div>
            ...
            <p className="text-sm text-zinc-400 mt-2">Deep analytical coaching and exclusive features</p>
          </div>
          <div className="space-y-3">
            <p className="text-xs font-medium text-zinc-500 uppercase tracking-wide">Everything in Free, plus:</p>
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 overflow-hidden">
        ...
        <div className="divide-y divide-zinc-800/50">
          <div className="grid grid-cols-3 px-6 py-3 bg-zinc-900 text-xs font-medium text-zinc-500 uppercase tracking-wide">
            <span>Feature</span>
            <span className="text-center">Free</span>
            <span className="text-center">Pro</span>
          </div>
          {planData?.features.map(f => (
            <div key={f.key} className="grid grid-cols-3 px-6 py-3 items-center hover:bg-zinc-800/30 transition-colors">
              <div>
                <p className="text-sm font-medium">{f.name}</p>
                <p className="text-xs text-zinc-500 hidden sm:block">{f.description}</p>
              </div>
      ...
      {user?.is_admin && (
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-6 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold">Admin: Manage Premium Access</h3>
          </div>
          <p className="text-sm text-zinc-400">
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            <input
              ...
              className="px-3 py-2 rounded-lg bg-zinc-800 border border-zinc-700 text-sm focus:outline-none focus:border-indigo-500"
            />
            <input
              type="text"
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={grantLoading || !grantUsername || !grantReason}
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-zinc-700 hover:bg-zinc-600 disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium transition-colors"
            >
              <X className="w-4 h-4" />
              Revoke Premium
            </button>
          ...
          {grantResult && (
            <div className={`px-4 py-2 rounded-lg text-sm ${
              grantResult.success ? "bg-emerald-500/10 text-emerald-300 border border-emerald-500/30" : "bg-red-500/10 text-red-300 border border-red-500/30"
            }`}>
              {grantResult.message}
            </div>
          )}
      ...
      {!isPremium && (
        <div className="rounded-2xl border border-amber-500/20 bg-gradient-to-r from-amber-950/20 via-zinc-900 to-zinc-900 p-6 text-center space-y-3">
          <Sparkles className="w-8 h-8 text-amber-400 mx-auto" />
          <h3 className="text-lg font-bold">Ready to level up your game?</h3>
