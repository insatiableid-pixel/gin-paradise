/**
 * Redis Coordinator Endurance Soak Proof.
 *
 * These tests only run when REDIS_URL is provided. They prove that two
 * real production-mode Gin Paradise server processes can survive a heavier
 * deterministic multi-room churn sequence with repeated crash/restart cycles
 * and continued gameplay after each recovery.
 */
...
const ROOM_SEEDS: RoomSeed[] = [
  { key: "ea1", ownerNode: "A", joinNode: "A", initialTurns: 0 },
  { key: "ea2", ownerNode: "A", joinNode: "B", initialTurns: 1 },
  { key: "ea3", ownerNode: "A", joinNode: "B", initialTurns: 2 },
  { key: "eb1", ownerNode: "B", joinNode: "B", initialTurns: 0 },
  { key: "eb2", ownerNode: "B", joinNode: "A", initialTurns: 1 },
  { key: "eb3", ownerNode: "B", joinNode: "A", initialTurns: 2 },
];

const CRASH_ORDER: NodeKey[] = ["A", "B", "A", "B"];

async function waitFor(condition: () => boolean, timeoutMs = 10_000): Promise<void> {
  const start = Date.now();
  while (!condition()) {
    if (Date.now() - start > timeoutMs) {
      throw new Error("Timed out waiting for endurance-soak state to propagate");
    }
    await delay(50);
  }
}
...
async function waitForSocketMessageSince(
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (socket.ws.readyState === socket.ws.CLOSED) {
      throw new Error("Socket closed before the expected endurance-soak message arrived.");
    }
  ...
  throw new Error(`Timed out waiting for endurance-soak websocket message. Observed: [${observed}]`);
}
...
async function waitForRecoveryWindow(
  ...
  await waitFor(() =>
    roomIds.every((roomId) => {
      ...
      const timerLease = probe.getLease(roomTimerLeaseName(roomId));
      return (
        !!room &&
...
async function setupLab(seedName: string): Promise<TestLab> {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `gin-paradise-endurance-soak-${seedName}-`));
  const databasePath = path.join(tempDir, "database.sqlite");
  const redisKeyPrefix = `endurance-soak:${seedName}:${crypto.randomUUID()}:`;
  const leaseEnv = {
  ...
  const nodeA = await startServer("endurance-soak-node-a", portA, databasePath, redisKeyPrefix, leaseEnv);
  const nodeB = await startServer("endurance-soak-node-b", portB, databasePath, redisKeyPrefix, leaseEnv);
...
async function runCrashPhase(
  ...
  await waitForRecoveryWindow(
    lab.probe,
    affectedRooms.map((room) => room.roomId),
    crashedHandle.nodeId,
  );
  await waitForHealth(
...
suite("RedisCoordinator endurance soak proof", () => {
  it("survives repeated seeded multi-room failover cycles and keeps play moving", async () => {
    const lab = await setupLab("full");

    try {
      const rooms: LiveRoomState[] = [];
      for (const [index, seed] of ROOM_SEEDS.entries()) {
        rooms.push(await createLiveRoomFromSeed(lab, seed, index + 1));
      }

      await waitFor(() => {
        const states = rooms.map((room) => lab.probe.getRoom(room.roomId));
        return (
          states.every(Boolean) &&
          states.filter((room) => room?.ownerNodeId === lab.nodeA.nodeId).length === 3 &&
          states.filter((room) => room?.ownerNodeId === lab.nodeB.nodeId).length === 3
        );
      }, 15_000);

      for (let cycle = 0; cycle < CRASH_ORDER.length; cycle += 1) {
        const crashNode = CRASH_ORDER[cycle];
        const survivorNode = oppositeNode(crashNode);
        const survivorHandle = getNode(lab, survivorNode);
        ...
        for (const room of rooms) {
          if (!getPlayer(room, room.activeRole).connected) {
            continue;
          }
          await playStockTurn(lab.probe, room);
        }
        ...
        await waitFor(() =>
          rooms.every((room) => {
            const state = lab.probe.getRoom(room.roomId);
            return (
              state?.ownerNodeId === survivorHandle.nodeId &&
              state?.timerOwnerNodeId === survivorHandle.nodeId
            );
          }),
          20_000,
        );
        ...
        const restarted = getNode(lab, crashNode);
        await waitForHealth(
          restarted.baseUrl,
          `restarted node ${crashNode} replication`,
          () => true,
          restarted.getLogs,
          (body) =>
            body.status === "healthy" &&
            body.coordinator?.healthy === true &&
            body.coordinator?.nodeId === restarted.nodeId &&
            (body.coordinator?.rooms ?? 0) >= rooms.length,
        );
      }

      const finalOwner = getNode(lab, "A");
      await waitFor(() =>
        rooms.every((room) => {
          const state = lab.probe.getRoom(room.roomId);
          return (
            state?.ownerNodeId === finalOwner.nodeId &&
            state?.timerOwnerNodeId === finalOwner.nodeId
          );
        }),
        20_000,
      );
