...
From the first bounded symbolic-evolution block (`#39`-`#50`):
...
| `gemini_gcli_0045`-`0049` | mostly `compute_eval_weight()` | Mostly discarded / tied | Pure stock-size schedules were too weak by themselves |
| `gemini_gcli_0044`, `0050` | mixed richer formulas | Discarded | More complex stock-heavy interactions regressed the knock lane |

**Immediate implications:**
- Keep the current winning family as the baseline:
  - `compute_eval_weight()` should still fundamentally trust direct EV more when `ev_gap` is large and trace data is richer
  - `compute_trace_confidence_multiplier()` should still fundamentally use non-linear decay with a protective high-deadwood floor
- The next campaign should prefer **local refinements of the winning family**
  over broad new directions
- Avoid pure stock-size-only schedules unless stock size is clearly a secondary
  interaction term inside an already strong formula
- Avoid over-parameterized polynomial / sqrt churn that adds complexity without
  clear signal
