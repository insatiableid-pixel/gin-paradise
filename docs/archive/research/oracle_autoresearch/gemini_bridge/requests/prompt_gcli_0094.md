...
## Semantic Mutation Surfaces (Directive 109/115)

These two helper functions in `train.py` are your **primary edit targets**.
Directive 115 has expanded their input surface to include richer game-state
features such as `turn_number` and detailed trace counts (`n_pickups`,
`n_discards`, `n_declines`).
...
| `gemini_gcli_0080` | coupled edits | `0.8055` but below margin | R4 reached raw 0.8055 but missed promotion margin, signaling local saturation of the old 4-input surface. |

**Immediate implications (Directive 115):**
- **Expanded Surface**: You now have access to `turn_number`, `n_pickups`, `n_discards`, and `n_declines`.
- **Winning Family**: 
  - `compute_eval_weight()`: EV-gap + trace-richness dynamic weighting.
  - `compute_trace_confidence_multiplier()`: Non-linear decay + deadwood floor + stock-size awareness.
- **R5 Objective**: Break through the 0.8010 plateau by using the **newly exposed features** (especially `turn_number` and `n_pickups`) to refine the winning logic.

### `compute_eval_weight(hero_deadwood, stock_size, n_trace_events, ev_gap, turn_number, n_pickups, n_discards, n_declines) → float`
...
- **Logic**: 
  ...
  ```
- **Opportunity**: add secondary interactions with `turn_number` or detailed pickup counts to refine late-game trust.

### `compute_trace_confidence_multiplier(n_trace_events, stock_size, hero_deadwood, turn_number, n_pickups, n_discards, n_declines) → float`
...
- **Logic**:
  ...
  ```
- **Opportunity**: refine the late-game floor or decay rate using `turn_number` or specific opponent pickup signals.
