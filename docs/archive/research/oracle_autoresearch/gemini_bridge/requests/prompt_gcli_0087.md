...
From the first, second, and third symbolic-evolution blocks:
...
| `gemini_gcli_0075` | `compute_eval_weight()` | `0.8052` but below margin | Progress-sensitive direct-EV trust looks promising and may combine well with the new trace floor |
| R2/R3 local variants | mixed scalar knobs / linear reversions | Mostly discarded | Family is stable; reverting to simpler linear ideas is weak |

**Immediate implications:**
- **Current Incumbent**: `gemini_gcli_0073` (`0.8010`)
- **Winning Family**: 
  - `compute_eval_weight()`: EV-gap + trace-richness dynamic weighting.
  - `compute_trace_confidence_multiplier()`: Non-linear (exponential) decay + protective floor, now with late-game stock-size awareness.
- Refine the **formulas** themselves now that the coefficients are better calibrated and the late-game floor signal is real.
- `TRACE_CONFIDENCE_DECAY = 0.10` is now the calibrated frontier.
- Look for **coupled interactions** between the two functions.
- Candidate `gemini_gcli_0075` suggests a unified edit touching both functions may provide the next jump.
- Avoid pure stock-size-only schedules; keep stock size as a supporting interaction term, not the whole idea.
...
- **Logic**: 
  ...
  ```
- **Opportunity**: further refine the shape of the trust curve or add secondary interactions (e.g., progress / stock depletion) without abandoning the winning EV-gap + trace-richness core.
...
- **Current (Dynamic)**: Uses exponential decay for risk reduction, a deadwood-scaled floor, and a stock-size-aware late-game floor boost.
- **Logic**:
  ```python
  ...
  ```
- **Opportunity**: refine the late-game floor behavior or couple it more intelligently to the eval-weight logic.
...
- Coupled edits that intentionally build on the `gemini_gcli_0073` + `gemini_gcli_0075` family
...
- Reverting the successful exponential / late-game floor logic back to simpler linear forms
